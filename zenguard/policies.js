...
export async function evaluatePolicy(userId, portfolio, positions) {
  const policy = userPolicies.get(userId);
...
if (config.dropThreshold) {
  ...
  // Evaluate chain lock
  if (config.chainLock) {
    const offChain = positions.find((p) => p.chain !== config.chainLock);
    if (offChain) {
      return {
        triggered: true,
        reason: `Position detected outside locked chain (${offChain.chain})`,
        token: offChain.symbol,
        amount: offChain.quantity?.float ?? 0,
        chain: offChain.chain,
      };
    }
  }
